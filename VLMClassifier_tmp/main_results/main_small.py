...
def main(
    ...
    # 시간 정보 추가
    output_path = output_path.replace(".jsonl", "_" + datetime.now().strftime("%m%d_%H%M%S") + ".jsonl")
    if os.path.exists(output_path):
        outputs = [json.loads(line) for line in open(output_path)]
        data = data[len(outputs) :]

    # init_prompt = "List the top-5 most likely object classes in this image in order of likelihood."
    init_prompt = None
    ...
    with open(output_path, "a") as f:
        for i in trange(0, len(data), batch_size):
            ...
            if multi_turn and not including_label:
                for item in batch:
                    item["predictions"] = []
                    item["excluded_classes"] = []
                    item["outputs"] = []  # 각 turn의 Q&A 저장용

                for turn in range(5):  # Get top-5 predictions
                    questions = []
                    for item in batch:
                        if turn == 0:
                            questions.append(init_prompt)
                        else:
                            excluded = ", ".join(item["excluded_classes"])
                            questions.append(f"{init_prompt} Excluding: {excluded}")
                    ...
                    for item, question, text in zip(batch, questions, generated_text):
                        if "mistral" in model_id:
                            pred = text.split("[/INST]")[-1].strip()
                        elif "blip" in model_id:
                        ...
                        item["excluded_classes"].append(pred)

                        # Q&A 형태로 저장
                        item["outputs"].append(f"Q: {question}\nA: {pred}")

                        # 마지막 turn이면 해당 Q&A만 별도 저장
                        if turn == 4:
                            item["output"] = f"Q: {question}\nA: {pred}"
                            item["pred"] = pred
...
@click.option("--multi_turn", default=False)
def entry(
    method,
    model_id,
    data_path,
